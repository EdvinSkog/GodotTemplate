...
## The controller will ignore mouse movements for head rotation.
...
func _ready() -> void:
	if PlayerManager.fpc == null:
		PlayerManager.fpc = self
	else:
		queue_free()
...
func toggle_active(option: bool, enable_cam: bool = true) -> void:
	if enable_cam:
...
func headbob_animation(moving: Vector2) -> void:
